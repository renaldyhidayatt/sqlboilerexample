...
func main() {
	...
	selectAuthorWithArticleEager(ctx, author.ID)
	selectAuthorWithArticle(ctx, author.ID)
}

func connectDB() *sql.DB {
	db, err := sql.Open("postgres", "postgres://@localhost:5432/sqlboilernyoba?sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	return db
...
func selectAuthorWithArticle(ctx context.Context, authorID int) {
	...
	fmt.Println("Select Author With Article")
...
func selectAuthorWithArticleEager(ctx context.Context, authorID int) {
	...
	fmt.Println("Select Author With Article Eager")
...
func selectAuthorWithArticleJoin(ctx context.Context, authorID int) {
	...
	fmt.Println("Select Author With Article Join")
	fmt.Println("")
